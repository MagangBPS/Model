...
# #PERBAIKI!! (Masih Plot Day n Night)
...
def load_and_preprocess_data(day_folder, target_size=(224, 224)):
    ...
    for day_image_path in day_images:
        ...
        if os.path.exists(sentinel_day_image_path):
...
original_model = DenseNet121(input_shape=(224,224,3), weights='imagenet', include_top=False)
densenet_day = add_prefix(original_model, 'densenet_day_')
...
def create_model(input_shape=(224, 224, 3)):
    ...
    dense = Dense(256, activation='relu', name='dense_256')(day_output)
    ...
    # Create the model
    model = Model(inputs=input_day, outputs=output, name='day_model')
...
x_day_s_maju, x_day_l_maju, y_maju = load_and_preprocess_data(day_folder_maju)

day_folder_tertinggal = '../Dataset/Day/TERTINGGAL'

x_day_s_tertinggal, x_day_l_tertinggal, y_tertinggal = load_and_preprocess_data(day_folder_tertinggal)
...
x_day_train, x_day_val, y_train, y_val = train_test_split(x_day, y, test_size=0.2, random_state=42)
...
plot_model(model, to_file='day_only_model_architecture_dropout.png', show_shapes=True, show_layer_names=True)
...
history = model.fit(
    ...
    epochs=EPOCHS,
    batch_size=8,
    validation_data=(x_day_val, y_val),
...
def evaluate_model(model, x_day, y_true, dataset_type="Validation"):
    # Predict the output
    y_pred_prob = model.predict(x_day)
...
evaluate_model(model, x_day_train, y_train, dataset_type="Training")

# Evaluate on validation data
evaluate_model(model, x_day_val, y_val, dataset_type="Validation")

# Save the final model
model.save(f'../h5_models/status_desa_densenet121_day_only_{EPOCHS}epochs_dropout.h5')
